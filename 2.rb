...
# Значение, хэш которого нужно посчитать - это инпут (пользовательский ввод), за которым следует положительное число
# (1, 2, 3 итд).
#
# Например, для инпута abcdef результатом будет число 609043, потому что MD5 хэш для abcdef609043 
# начинается с 5 нулей (000001dbbfa...)
...
input = gets.chomp
for i in (1..)
	if ((Digest::MD5.hexdigest (input+i.to_s)).to_s)[0..4] == '00000'
		puts i
		break
	end
end
